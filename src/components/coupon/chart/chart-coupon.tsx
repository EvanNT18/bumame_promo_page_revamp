  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center p-6 lg:p-4">
        ...
        <div className="flex flex-col w-full h-full max-w-[75%] max-h-[70%] bg-opacity-90 rounded-lg overflow-hidden cursor-pointer">
          ...
          <div className="flex-1">
            <p
              title={description}
              className="text-xs sm:max-w-[180px] text-gray-600 line-clamp-2"
            >
              {description}
            </p>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              ...
              >
                {couponCode.length > 8
                  ? `${couponCode.slice(0, 6)}...`
                  : couponCode}
              </span>
            </div>
            <Link
              target="_blank"
              href={`https://api.whatsapp.com/send/?phone=6281119088808&text=Hi+Bumame%2C+I+want+to+redeem+my+code%3A+${encodeURIComponent(
                couponCode
              )}`}
